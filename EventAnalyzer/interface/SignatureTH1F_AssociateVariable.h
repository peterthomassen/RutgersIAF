#ifndef SignatureTH1F_AssociateVariable_h
#define SignatureTH1F_AssociateVariable_h
...
template <typename T>
class SignatureTH1F_AssociateVariable : public SignatureTH1F {
public:
  
 SignatureTH1F_AssociateVariable(const char* name, TString varname, TString productname, TString associatename, const char* title = "N Distribution", int nbins = 20, double xmin = -0.5, double xmax = 19.5):SignatureTH1F(name,title,nbins,xmin,xmax),m_varname(varname),m_associatename(associatename){ m_productnames.push_back(productname);}
    
  SignatureTH1F_AssociateVariable(TH1F h):SignatureTH1F(h) {}
  SignatureTH1F_AssociateVariable():SignatureTH1F() {}
  ...
  Int_t Fill(BaseHandler* handler)
  {
	...
	T value;
	bool isSet = associate->getVariable(m_varname,value);
