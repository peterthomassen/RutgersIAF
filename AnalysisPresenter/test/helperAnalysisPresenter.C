...
/*		if(dilep) {
		...
		if(!assembler->getMode("noTTsystematics")) ttbarF->addFlatUncertainty("ttbarFakeRate", 0.5);
		mc.push_back(ttbarF);
	...
	if(dilep && !onlyTTF) {
		...
		c->addWeight("(NLEPTONS[0] == NLEPTONS[0]) * ((NGOODJETS[0] == 0) * 1.0 + (NGOODJETS[0] == 1) * 0.93 + (NGOODJETS[0] == 2) * 0.92 + (NGOODJETS[0] == 3) * 0.93 + (NGOODJETS[0] == 4) * 1.09 + (NGOODJETS[0] == 5) * 1.27 + (NGOODJETS[0] == 6) * 1.61 + (NGOODJETS[0] > 6) * 1)");
		mc.push_back(c);
