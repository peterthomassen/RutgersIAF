...
void init(Assembler* assembler, std::string backgroundModel = "proxy") {
	...
	assembler->addBundle(new Bundle("background", "MMfakes", false, kYellow - 7));
...
void prepare(Assembler* assembler) {
	...
	for(const auto &bundleName : {"TrackFakes", "PhotonFakes", "MMfakes"}) {
	...
	for(const auto &bundleName : {"TrackFakes", "PhotonFakes", "WZ", "Higgs", "Rare MC", "MMfakes"}) {
		Bundle* bundle = assembler->getBundle(bundleName);
		if(bundle->getComponents().size() > 0) {
...
void setupData(Assembler* assembler, bool dilep = false, int fakeMode = 0, bool applyEventVetos = false) {
	...
	PhysicsContribution* dataMuMu_C = 0;
	PhysicsContribution* dataMuMu_D = 0;
	PhysicsContribution* dataMuMu_E = 0;
	PhysicsContribution* dataElEl_B = 0;
	PhysicsContribution* dataElEl_C = 0;
	PhysicsContribution* dataElEl_D = 0;
	PhysicsContribution* dataElEl_E = 0;
	PhysicsContribution* dataMuEl_B = 0;
	PhysicsContribution* dataMuEl_C = 0;
	PhysicsContribution* dataMuEl_D = 0;
	PhysicsContribution* dataMuEl_E = 0;
	...
	std::string TREE="treeR";
	  ...
	  if(data2mu){	  
	    dataMuMu_B = new PhysicsContribution("data","/cms/multilepton/akapoor/Samples/DoubleMuon_2016B/DoubleMuon_2016B"+ infix + suffix,5870/3,"15.9/fb@13TeV",false,TREE);
	    
	    dataMuMu_C = new PhysicsContribution("data","/cms/multilepton/akapoor/Samples/DoubleMuon_2016C/DoubleMuon_2016C"+ infix + suffix,2646/3,"15.9/fb@13TeV",false,TREE);
	    
	    dataMuMu_D = new PhysicsContribution("data","/cms/multilepton/akapoor/Samples/DoubleMuon_2016D/DoubleMuon_2016D"+ infix + suffix,4353/3, "15.9/fb@13TeV",false,TREE);
	    
	    dataMuMu_E = new PhysicsContribution("data","/cms/multilepton/akapoor/Samples/DoubleMuon_2016E/DoubleMuon_2016E"+ infix + suffix,3050/3, "15.9/fb@13TeV",false,TREE);
	    
	    data.push_back(dataMuMu_B);	  
	    data.push_back(dataMuMu_C);
	    data.push_back(dataMuMu_D);
	    data.push_back(dataMuMu_E);
	  }
	  
	  if(data2e){
	    
	    dataElEl_B = new PhysicsContribution("data","/cms/multilepton/akapoor/Samples/DoubleEG_2016B/DoubleEG_2016B"+ infix + suffix,5889/3, "15.9/fb@13TeV",false,TREE);

	    dataElEl_C = new PhysicsContribution("data","/cms/multilepton/akapoor/Samples/DoubleEG_2016C/DoubleEG_2016C"+ infix + suffix,2644/3, "15.9/fb@13TeV",false,TREE);
	    
	    dataElEl_D = new PhysicsContribution("data","/cms/multilepton/akapoor/Samples/DoubleEG_2016D/DoubleEG_2016D"+ infix + suffix,4340/3, "15.9/fb@13TeV",false,TREE);
	    
	    dataElEl_E = new PhysicsContribution("data","/cms/multilepton/akapoor/Samples/DoubleEG_2016E/DoubleEG_2016E"+ infix + suffix,3050/3, "15.9/fb@13TeV",false,TREE);
	    
	    data.push_back(dataElEl_B);	  
	    data.push_back(dataElEl_C);
	    data.push_back(dataElEl_D);
	    data.push_back(dataElEl_E);
	  }
	  
	  if(data1mu1e){
	    dataMuEl_B = new PhysicsContribution("data", "/cms/multilepton/akapoor/Samples/MuonEG_2016B/MuonEG_2016B" + infix + suffix, 5871/3, "15.9/fb@13TeV",false,TREE);
	    
	    dataMuEl_C = new PhysicsContribution("data", "/cms/multilepton/akapoor/Samples/MuonEG_2016C/MuonEG_2016C" + infix + suffix, 2646/3, "15.9/fb@13TeV",false,TREE);
	    
	    dataMuEl_D = new PhysicsContribution("data", "/cms/multilepton/akapoor/Samples/MuonEG_2016D/MuonEG_2016D" + infix + suffix, 4353/3, "15.9/fb@13TeV",false,TREE);
	    
	    dataMuEl_E = new PhysicsContribution("data", "/cms/multilepton/akapoor/Samples/MuonEG_2016E/MuonEG_2016E" + infix + suffix, 3050/3, "15.9/fb@13TeV",false,TREE);
	    
	    data.push_back(dataMuEl_B);	  
	    data.push_back(dataMuEl_C);
	    data.push_back(dataMuEl_D);
	    data.push_back(dataMuEl_E);
	  }

	  for(auto &contribution : data) {
	    
	    if(data2e&&data2mu&&data1mu1e)
	      contribution->addWeight("TRIGGERACCEPT");
	    else
	      contribution->addWeight("(NIsoDiEleHLT>0)");
	    
	    contribution->setAlias("METraw","MET");
	    contribution->setAlias("METPXraw","METPX");
	    contribution->setAlias("METPYraw","METPY");
	    //contribution->addWeight("NIsoDiMuHLT");
	    contribution->addWeight("NLIGHTLEPTONS[0] == Sum$(ISTIGHTMATRIXMUON) + Sum$(ISTIGHTMATRIXELECTRON)");
	  ...
	  TFile* f = new TFile("/users/h2/schauhan/PileUpHist/272007-275376_276315_276811_10.246fb_PileupHistogram_xsec69200.root");
	  if(f->IsZombie()) {
	  ...
	  if(assembler->getMode("fullPrecision")) {
	    TFile* fUnc = new TFile("/users/h2/schauhan/PileUpHist/272007-275376_276315_276811_10.246fb_Pileup+5%Histogram_xsec69200.root");
	    if(fUnc->IsZombie()) {
...
void setupBackgroundMC(Assembler* assembler, bool dilep = false, bool ttbar = true) {
	...
	if(ttbar && !assembler->getMode("backgroundModel_MM")) {
		PhysicsContribution* ttbarF = new PhysicsContribution("backgroundMC", prefix + "TTTo2L2Nu_2__061616_090554/TTTo2L2Nu" + infix + suffix, xsec_ttF, "ttbar", false, "treeR", kAzure + 1, 0.1);
	...
	for(auto &contribution : mc) {
	  contribution->setAlias("METraw","METnoSmearing");
          contribution->setAlias("METPXraw","METPX");
          contribution->setAlias("METPYraw","METPY");
	  ...
	  if(data2e&&data2mu&&data1mu1e)
	    ...
	    contribution->addWeight("(DIELTRIGTHRESHOLD)");
	  //contribution->addWeight("DIMUTRIGTHRESHOLD");
	  //contribution->addWeight("DIELTRIGTHRESHOLD");
	  //contribution->addWeight("MUEGCOMBINEDTHRESHOLD");
...
void setupBackgroundDD(Assembler* assembler, TString option = "", bool syst = true) {
  ...
  std::string infix = "";
  //std::string suffix = ".data.root";
  std::vector<PhysicsContribution*> fakeTracks;
  std::vector<PhysicsContribution*> fakePhotons;
  std::vector<PhysicsContribution*> MM;
  PhysicsContribution* dataMuMu_B = 0;
  PhysicsContribution* dataMuMu_C = 0;
  PhysicsContribution* dataMuMu_D = 0;
  PhysicsContribution* dataMuMu_E = 0;
  ...
  int data2e=1;
  int data1mu1e=1;
  //Below: Trees for fakeTracks
  if(data2mu){
  dataMuMu_B = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleMuon_2016B/DoubleMuon_2016B"+infix+suffix,assembler->getLumi(), "fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1,0);
  dataMuMu_C = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleMuon_2016C/DoubleMuon_2016C"+infix+suffix,assembler->getLumi(),"fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1,0);
  dataMuMu_D = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleMuon_2016D/DoubleMuon_2016D"+infix+suffix,assembler->getLumi(), "fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1,0);
  dataMuMu_E = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleMuon_2016E/DoubleMuon_2016E" +infix+suffix,assembler->getLumi(),"fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1,0);
  fakeTracks.push_back(dataMuMu_B);
  fakeTracks.push_back(dataMuMu_C);
  fakeTracks.push_back(dataMuMu_D);
  fakeTracks.push_back(dataMuMu_E);
  }

  if(data2e){
  dataElEl_B = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/DoubleEG_2016B/DoubleEG_2016B" + infix + suffix, assembler->getLumi(), "fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1);
  fakeTracks.push_back(dataElEl_B);
  dataElEl_D = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleEG_2016D/DoubleEG_2016D"+ infix + suffix,assembler->getLumi(), "fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1);
  fakeTracks.push_back(dataElEl_D);
  dataElEl_C = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleEG_2016C/DoubleEG_2016C"+ infix + suffix,assembler->getLumi(), "fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1);
  fakeTracks.push_back(dataElEl_C);
  dataElEl_E = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleEG_2016E/DoubleEG_2016E"+ infix + suffix,assembler->getLumi(), "fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1);
  fakeTracks.push_back(dataElEl_E);
  }

  if(data1mu1e){
  dataMuEl_B = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/MuonEG_2016B/MuonEG_2016B" + infix + suffix, assembler->getLumi(), "fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1);
  fakeTracks.push_back(dataMuEl_B);
  dataMuEl_C = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/MuonEG_2016C/MuonEG_2016C" + infix + suffix, assembler->getLumi(), "fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1);
  fakeTracks.push_back(dataMuEl_C);
  dataMuEl_D = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/MuonEG_2016D/MuonEG_2016D" + infix + suffix, assembler->getLumi(), "fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1);
  fakeTracks.push_back(dataMuEl_D);
  dataMuEl_E = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/MuonEG_2016E/MuonEG_2016E" + infix + suffix, assembler->getLumi(), "fakeTracks",false,"treeRfakeTracks",(option == "justTracks") ? kWhite : -1);
  fakeTracks.push_back(dataMuEl_E);
  }
  //////////////////////Trees for fakePhotons below
  if(data2mu){
    dataMuMu_B = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/DoubleMuon_2016B/DoubleMuon_2016B" + infix + suffix,assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    dataMuMu_C = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleMuon_2016C/DoubleMuon_2016C"+infix+suffix,assembler->getLumi(),"fakePhotons",false,"treeRfakePhotons");
    dataMuMu_D = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/DoubleMuon_2016D/DoubleMuon_2016D" + infix + suffix, assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    dataMuMu_E = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/DoubleMuon_2016E/DoubleMuon_2016E" + infix + suffix, assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    fakePhotons.push_back(dataMuMu_B);
    fakePhotons.push_back(dataMuMu_C);
    fakePhotons.push_back(dataMuMu_D);
    fakePhotons.push_back(dataMuMu_E);
  }

  if(data2e){
    dataElEl_B = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/DoubleEG_2016B/DoubleEG_2016B" + infix + suffix, assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    fakePhotons.push_back(dataElEl_B);
    dataElEl_D = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleEG_2016D/DoubleEG_2016D"+ infix + suffix,assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    fakePhotons.push_back(dataElEl_D);
    dataElEl_C = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleEG_2016C/DoubleEG_2016C"+ infix + suffix,assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    fakePhotons.push_back(dataElEl_C);
    dataElEl_E = new PhysicsContribution("backgroundDD","/cms/multilepton/akapoor/Samples/DoubleEG_2016E/DoubleEG_2016E"+ infix + suffix,assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    fakePhotons.push_back(dataElEl_E);
  }

  if(data1mu1e){
    dataMuEl_B = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/MuonEG_2016B/MuonEG_2016B" + infix + suffix, assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    fakePhotons.push_back(dataMuEl_B);
    dataMuEl_C = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/MuonEG_2016C/MuonEG_2016C" + infix + suffix, assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    fakePhotons.push_back(dataMuEl_C);
    dataMuEl_D = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/MuonEG_2016D/MuonEG_2016D" + infix + suffix, assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    fakePhotons.push_back(dataMuEl_D);
    dataMuEl_E = new PhysicsContribution("backgroundDD", "/cms/multilepton/akapoor/Samples/MuonEG_2016E/MuonEG_2016E" + infix + suffix, assembler->getLumi(), "fakePhotons",false,"treeRfakePhotons");
    fakePhotons.push_back(dataMuEl_E);
  }
 /////////////////////
 ...
 if(assembler->getMode("backgroundModel_proxy")) {
   for(auto &contribution : fakeTracks) {
     contribution->setAlias("METraw","MET");
     contribution->setAlias("METPXraw","METPX");
     contribution->setAlias("METPYraw","METPY");
     //contribution->addWeight("NIsoDiMuHLT>0");
     ...
     if(data2e&&data2mu&&data1mu1e)
       ...
       contribution->addWeight("(NIsoMuEleHLT>0)");
     contribution->addWeight("NLIGHTLEPTONS[0] == Sum$(ISTIGHTMATRIXMUON) + Sum$(ISTIGHTMATRIXELECTRON) + Sum$(fakeRoleGOODMUONS > 0) + Sum$(fakeRoleGOODELECTRONS > 0)");
     contribution->addWeight(nVertexWeight);
     //contribution->addWeight("contribution.getLumi()");
     contribution->addRelativeUncertainty("fakePileupWeight", TString::Format("1 - 1/(%s)", nVertexWeight.Data()));
     applyUncertaintiesAndScaleFactors(assembler, contribution);
     if(option != "noTracks" && option != "justTaus" && option != "justPhotons" && option != "fakeTaus2L") {
       assembler->getBundle("TrackFakes")->addComponent(contribution);
       assembler->addContribution(contribution);
     }
   }
 }
 ...
 if(assembler->getMode("backgroundModel_proxy") || assembler->getMode("backgroundModel_MM")) {
   for(auto &contribution : fakePhotons) {
     //   PhysicsContribution* contribution = new PhysicsContribution("backgroundDD", prefix + body + suffix, 4300, "contribution", false, "treeRcontribution");
     contribution->setAlias("METraw","MET");
     contribution->setAlias("METPXraw","METPX");
     contribution->setAlias("METPYraw","METPY");
     //contribution->addWeight("NIsoDiMuHLT>0");
     ...
     if(option != "noPhotons" && option != "justTracks" && option != "justTaus" && option != "fakeTaus2L") {
       assembler->getBundle("PhotonFakes")->addComponent(contribution);
       assembler->addContribution(contribution);
     }
   }
 }
 
 ////Below: Trees for Matrix Method
 if(data2mu){
   dataMuMu_B = new PhysicsContribution("backgroundMM", "/cms/multilepton/akapoor/Samples/DoubleMuon_2016B/DoubleMuon_2016B" + infix + suffix,assembler->getLumi(), "MM",false,"treeR");
   dataMuMu_C = new PhysicsContribution("backgroundMM","/cms/multilepton/akapoor/Samples/DoubleMuon_2016C/DoubleMuon_2016C"+infix+suffix,assembler->getLumi(),"MM",false,"treeR");
   dataMuMu_D = new PhysicsContribution("backgroundMM", "/cms/multilepton/akapoor/Samples/DoubleMuon_2016D/DoubleMuon_2016D" + infix + suffix, assembler->getLumi(), "MM",false,"treeR");
   dataMuMu_E = new PhysicsContribution("backgroundMM", "/cms/multilepton/akapoor/Samples/DoubleMuon_2016E/DoubleMuon_2016E" + infix + suffix, assembler->getLumi(), "MM",false,"treeR");
   MM.push_back(dataMuMu_B);
   MM.push_back(dataMuMu_C);
   MM.push_back(dataMuMu_D);
   MM.push_back(dataMuMu_E);
 }

 if(data2e){
   dataElEl_B = new PhysicsContribution("backgroundMM", "/cms/multilepton/akapoor/Samples/DoubleEG_2016B/DoubleEG_2016B" + infix + suffix, assembler->getLumi(), "MM",false,"treeR");
   MM.push_back(dataElEl_B);
   dataElEl_D = new PhysicsContribution("backgroundMM","/cms/multilepton/akapoor/Samples/DoubleEG_2016D/DoubleEG_2016D"+ infix + suffix,assembler->getLumi(), "MM",false,"treeR");
   MM.push_back(dataElEl_D);
   dataElEl_C = new PhysicsContribution("backgroundMM","/cms/multilepton/akapoor/Samples/DoubleEG_2016C/DoubleEG_2016C"+ infix + suffix,assembler->getLumi(), "MM",false,"treeR");
   MM.push_back(dataElEl_C);
   dataElEl_E = new PhysicsContribution("backgroundMM","/cms/multilepton/akapoor/Samples/DoubleEG_2016E/DoubleEG_2016E"+ infix + suffix,assembler->getLumi(), "MM",false,"treeR");
   MM.push_back(dataElEl_E);
 }
 if(data1mu1e){
   dataMuEl_B = new PhysicsContribution("backgroundMM", "/cms/multilepton/akapoor/Samples/MuonEG_2016B/MuonEG_2016B" + infix + suffix, assembler->getLumi(), "MM",false,"treeR");
   MM.push_back(dataMuEl_B);
   dataMuEl_C = new PhysicsContribution("backgroundMM", "/cms/multilepton/akapoor/Samples/MuonEG_2016C/MuonEG_2016C" + infix + suffix, assembler->getLumi(), "MM",false,"treeR");
   MM.push_back(dataMuEl_C);
   dataMuEl_D = new PhysicsContribution("backgroundMM", "/cms/multilepton/akapoor/Samples/MuonEG_2016D/MuonEG_2016D" + infix + suffix, assembler->getLumi(), "MM",false,"treeR");
   MM.push_back(dataMuEl_D);
   dataMuEl_E = new PhysicsContribution("backgroundMM", "/cms/multilepton/akapoor/Samples/MuonEG_2016E/MuonEG_2016E" + infix + suffix, assembler->getLumi(), "MM",false,"treeR");
   MM.push_back(dataMuEl_E);
 }
 ////// Matrix Method
 if(assembler->getMode("backgroundModel_MM")) {
   
   for(auto &contribution :MM){
     ...
     contribution->addWeight(
                             "(NGOODMUONS==3)"//3mu                                                                                                                                                                     
                             "*("
                             "(MMweight(Alt$(PTGOODMUONS[0],0),Alt$(ETAGOODMUONS[0],0),Alt$(PTGOODMUONS[1],0),Alt$(ETAGOODMUONS[1],0),Alt$(PTGOODMUONS[2],0),Alt$(ETAGOODMUONS[2],0),((Alt$(JETNOOFCONSTGOODMUONS[0],0)>11.5)*100+(Alt$(JETNOOFCONSTGOODMUONS[1],0)>11.5)*10+(Alt$(JETNOOFCONSTGOODMUONS[2],0)>11.5)),NRECOVERTICES[0],(Alt$(ISTIGHTMATRIXMUON[0],0)*100000+Alt$(ISTIGHTMATRIXMUON[1],0)*10000+Alt$(ISTIGHTMATRIXMUON[2],0)*1000+222)))"
                             ")"
                             "+(NGOODMUONS==2)"//2mu1e                                                                                                                                                                                       
                             "*("
                             "(MMweight(Alt$(PTGOODMUONS[0],0),Alt$(ETAGOODMUONS[0],0),Alt$(PTGOODMUONS[1],0),Alt$(ETAGOODMUONS[1],0),Alt$(PTGOODELECTRONS[0],0),Alt$(ETAGOODELECTRONS[0],0),((Alt$(JETNOOFCONSTGOODMUONS[0],0)>11.5)*100+(Alt$(JETNOOFCONSTGOODMUONS[1],0)>11.5)*10+(Alt$(CONENDR03TO07GOODELECTRONS[0],0)>5.5)),NRECOVERTICES[0],(Alt$(ISTIGHTMATRIXMUON[0],0)*100000+Alt$(ISTIGHTMATRIXMUON[1],0)*10000+Alt$(ISTIGHTMATRIXELECTRON[0],0)*1000+221)))"
                             ")"
                             "+(NGOODELECTRONS==2)"//2e1mu                                                                                                                                                                                   
                             "*("
                             "(MMweight(Alt$(PTGOODMUONS[0],0),Alt$(ETAGOODMUONS[0],0),Alt$(PTGOODELECTRONS[0],0),Alt$(ETAGOODELECTRONS[0],0),Alt$(PTGOODELECTRONS[1],0),Alt$(ETAGOODELECTRONS[1],0),((Alt$(JETNOOFCONSTGOODMUONS[0],0)>11.5)*100+(Alt$(CONENDR03TO07GOODELECTRONS[0],0)>5.5)*10+(Alt$(CONENDR03TO07GOODELECTRONS[1],0)>5.5)),NRECOVERTICES[0],(Alt$(ISTIGHTMATRIXMUON[0],0)*100000+Alt$(ISTIGHTMATRIXELECTRON[0],0)*10000+Alt$(ISTIGHTMATRIXELECTRON[1],0)*1000+211)))"
                             ")"
                             "+(NGOODELECTRONS==3)"//3e                                                                                                                                                                                      
                             "*("
                             "(MMweight(Alt$(PTGOODELECTRONS[0],0),Alt$(ETAGOODELECTRONS[0],0),Alt$(PTGOODELECTRONS[1],0),Alt$(ETAGOODELECTRONS[1],0),Alt$(PTGOODELECTRONS[2],0),Alt$(ETAGOODELECTRONS[2],0),((Alt$(CONENDR03TO07GOODELECTRONS[0],0)>5.5)*100+(Alt$(CONENDR03TO07GOODELECTRONS[1],0)>5.5)*10+(Alt$(CONENDR03TO07GOODELECTRONS[2],0)>5.5)),NRECOVERTICES[0],(Alt$(ISTIGHTMATRIXELECTRON[0],0)*100000+Alt$(ISTIGHTMATRIXELECTRON[1],0)*10000+Alt$(ISTIGHTMATRIXELECTRON[2],0)*1000+111)))"
                             ")");

     assembler->getBundle("MMfakes")->addComponent(contribution);
     assembler->addContribution(contribution);
   }
 }
}
...
void setupFakeRates(Assembler* assembler) {
  if(assembler->getMode("backgroundModel_proxy")) {
    ...
    assembler->setFakeRate("nTrackFakeMuons",
			   "("
			   "(NGOODMUONS[0] == 1 || NGOODMUONS[0] == 2 || NGOODMUONS[0] == 4 || NGOODMUONS[0] == 3)*((Min$(PTGOODMUONS)<=15) * 0.01128 + (Min$(PTGOODMUONS)>15) *0.0158)" // measurement done here
    ...
    assembler->setFakeRate("nTrackFakeElectrons",
			   "("
			   "(NGOODELECTRONS[0] == 1 || NGOODELECTRONS[0] == 2 || NGOODELECTRONS[0] == 4 || NGOODELECTRONS[0] == 3)*((Min$(PTGOODELECTRONS) <=15) * 0.0054 +  (Min$(PTGOODELECTRONS)>15 && Min$(PTGOODELECTRONS)<=20) * 0.0136 + (Min$(PTGOODELECTRONS)>20 && Min$(PTGOODELECTRONS)<=25)* 0.0272 + (Min$(PTGOODELECTRONS)>25 && Min$(PTGOODELECTRONS)<=30)* 0.026+(Min$(PTGOODELECTRONS)>30) * 0.038)"// measurement done here
			   ")"
			   " * (nTrackFakeElectrons[0] + nTrackFakeMuons[0] == 1)" // disable multiple proxies (precaution to avoid problems like with taus)
			   );
  }
  ...
  if(assembler->getMode("backgroundModel_proxy")){
    ...
    assembler->setFakeRate("nPhotonFakeMuons",
			   "("
			   "(NGOODMUONS[0] == 1 || NGOODMUONS[0] == 3 || NGOODMUONS[0] == 2 || NGOODMUONS[0] == 4) *((Min$(PTGOODMUONS) <=15)* 0.0072 + (Min$(PTGOODMUONS) > 15&&Min$(PTGOODMUONS)<=20)*0.009 + (Min$(PTGOODMUONS) >20 && Min$(PTGOODMUONS) <=30)*0.014 + (Min$(PTGOODMUONS) >30)*0.028)" // measurement done here
			   ")"
    ...
    assembler->setFakeRate("nPhotonFakeElectrons",
			   "("
			   "(NGOODELECTRONS[0] == 1 || NGOODELECTRONS[0] == 3 || NGOODMUONS[0] == 2 || NGOODMUONS[0] == 4) * ((Min$(PTGOODELECTRONS) <=15) * 0.022 + (Min$(PTGOODELECTRONS) > 15&&Min$(PTGOODELECTRONS)<=20)*0.031 + (Min$(PTGOODELECTRONS) > 20&&Min$(PTGOODELECTRONS)<=25)*0.038 + (Min$(PTGOODELECTRONS) > 25&&Min$(PTGOODELECTRONS)<=30)*0.044+(Min$(PTGOODELECTRONS) > 30)*0.058)" // measurement done here                                                                                   
			   ")"
			   " * Alt$(Sum$(abs(goodElectronsDRgoodLeptons - 0.5) < 0.1 && fakeRoleGOODELECTRONS > 0) > 0, 0)" // only stuff emitted close to leptons
			   " * (nPhotonFakeElectrons[0] + nPhotonFakeMuons[0] == 1)" // disable multiple proxies (precaution to avoid problems like with taus)
			   );
  }
  //Photon fake rates for Matrix Method
  if(assembler->getMode("backgroundModel_MM")){
    assembler->setFakeRate("nPhotonFakeMuons",
                           "("
                           "(NGOODMUONS[0] == 1 || NGOODMUONS[0] == 2|| NGOODMUONS[0] == 4 ||NGOODMUONS[0] == 3)"
                           " *((Min$(PTGOODMUONS) <=15)* 0.0072 + (Min$(PTGOODMUONS) > 15&&Min$(PTGOODMUONS)<=20)*0.009 + (Min$(PTGOODMUONS) >20 && Min$(PTGOODMUONS) <=30)*0.014 + (Min$(PTGOODMUONS) >30)*0.028)" // measurement done here  
                           ")"
                           " * Alt$(Sum$(abs(goodMuonsDRgoodLeptons - 0.5) < 0.1 && fakeRoleGOODMUONS > 0) > 0, 0)" // only stuff emitted close to leptons                                                                                    
                           " * (nPhotonFakeElectrons[0] + nPhotonFakeMuons[0] == 1)" // disable multiple proxies (precaution to avoid problems like with taus)                                                                                
                           );
    assembler->setFakeRate("nPhotonFakeElectrons",
                           "("
                           "(NGOODELECTRONS[0] == 1 || NGOODELECTRONS[0] == 2 || NGOODELECTRONS[0] == 4 || NGOODELECTRONS[0] == 3)"
                           "*((Min$(PTGOODELECTRONS) <=15) * 0.014 + (Min$(PTGOODELECTRONS) > 15&&Min$(PTGOODELECTRONS)<=20)*0.031 + (Min$(PTGOODELECTRONS) > 20&&Min$(PTGOODELECTRONS)<=25)*0.038 + (Min$(PTGOODELECTRONS) > 25&&Min$(PTGOODELECTRONS)<=30)*0.044+(Min$(PTGOODELECTRONS) > 30)*0.058)" // measurement done here                                                                                                                                                           
                           ")"
                           " * Alt$(Sum$(abs(goodElectronsDRgoodLeptons - 0.5) < 0.1 && fakeRoleGOODELECTRONS > 0) > 0, 0)" // only stuff emitted close to leptons                                                                            
                           " * (nPhotonFakeElectrons[0] + nPhotonFakeMuons[0] == 1)" // disable multiple proxies (precaution to avoid problems like with taus)                                                                                
                           );
