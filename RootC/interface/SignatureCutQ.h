...
class SignatureCutQ : public SignatureCut {
 public:
   SignatureCutQ(TString productname, double nlow = -9999, double nhigh=9999,const char* name="sigcutq") : SignatureCut(name),m_nlow(nlow),m_nhigh(nhigh){ m_productnames.push_back(productname);}
  ...
  void addProductName(TString productname) {m_productnames.push_back(productname);}
  std::vector<TString> getProductNames() {return m_productnames;}
...
{
  double dummy = 0;
  for(std::vector<TString>::const_iterator it = m_productnames.begin(); it < m_productnames.end(); it++)
  {
