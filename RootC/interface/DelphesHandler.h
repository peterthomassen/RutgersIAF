//////////////////////////////////////////////////////
//
//  This Class Inherits from DelphesTreeBase and lets you 
//  Use all of the SkimTree functionality without actually
//  looking through the mass of code
//
//////////////////////////////////////////////////////

#ifndef DelphesHandler_h
#define DelphesHandler_h
...
class DelphesHandler: public BaseHandler, public DelphesTreeBase {
 public:
  //default constructor
  DelphesHandler(TTree *anatree=0, TString ofname="SigHanOutput.root");
  //virtual destructor
  virtual ~DelphesHandler();
  ...
  double getPFMET() {return m_met->Pt();}
  double getMT(){return m_MT;}
  double getMLL(){return m_MLL;}
  double getPU(){return (double)m_recovertices.size();}
  ...
  void setSameDzCut(float c){m_sameDzCut = c;}
  void setSameD0Cut(float c){m_sameD0Cut = c;}

  void addDumpEvent(long);
 ...
 private:
  ...
  double calcTotalTrackIso(SignatureObjectRecoTrack*,int);
  int matchToRecoTrack(SignatureObjectRecoTrack*);
  int matchToRecoTrack(Jet*);
  double calcTauTotalIso(Jet*, int);
  int countJetConstituents(Jet*);

  std::vector<long> m_dumpList;
 ...
 protected:
  ...
  ClassDef(DelphesHandler,1);

};

#endif //#ifdef DelphesHandler_cxx
