...
class SignatureTH2F_HTvsWSumPt : public SignatureTH2F {
 public:
  SignatureTH2F_HTvsWSumPt(const char* name, const char * productname, const char* title = "HT vs WSumPt Distribution", Int_t nbinsX = 200, Double_t minX = 0, Double_t maxX = 2500, Int_t nbinsY = 200, Double_t minY = 0, Double_t maxY = 2500):SignatureTH2F(name,title,nbinsX,minX,maxX,nbinsY,minY,phiY){ }
...
inline Int_t SignatureTH2F_HTvsWSumPt::Fill(BaseHandler* handler)
{
	double WSumPt = (handler->getGoodRecoVertices().size() > 0)
		? max(handler->getGoodRecoVertices()[0]->getWSumPt(), 0.0)
		: -1000;
