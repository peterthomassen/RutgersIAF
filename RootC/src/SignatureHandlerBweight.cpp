...
void SignatureHandler::calcPhysicsWeightWithBjets()
{
  ...
  //b-tag reweight//
  //////////////////
  int nbquarks=0,ncquarks=0,nlight=0;
  //int nbTagged=0,ncTagged=0,nlTagged=0;

  vector<int> lightTagged;
  vector<int> bTagged;
  vector<int> cTagged;

  //second figure out how many things we have
  if(m_isMC){
    for(int i = 0; i < (int)m_good_jets.size(); i++){
      int jetPDGID = ((SignatureObjectJet*)m_good_jets[i])->getGenParton_pdgID();
      if(jetPDGID == -999)continue;
      bool tagged = isBTagged((SignatureObjectJet*)m_good_jets[i]);
      if(abs(jetPDGID)==5){
	int t = 0;
	nbquarks++;
	if(tagged)t = 1;
	bTagged.push_back(t);
      }else if(abs(jetPDGID)==4){
	int t = 0;
	ncquarks++;
	if(tagged)t = 1;
	cTagged.push_back(t);
	...
	nlight++;
	if(tagged)t=1;
	lightTagged.push_back(t);
      }
    }
    
    //second get SF based on association
    vector<float> sfLight = getLightScaleFactors();
    vector<float> sfB = getBScaleFactors();
    vector<float> sfC = getCScaleFactors();
    
    vector<float> sfs = sfLight;
    sfs.insert(sfs.end(),sfC.begin(),sfC.end());
    sfs.insert(sfs.end(),sfB.begin(),sfB.end());
    
    vector<int> tags = lightTagged;
    tags.insert(tags.end(),cTagged.begin(),cTagged.end());
    tags.insert(tags.end(),bTagged.begin(),bTagged.end());
    
    m_bTagged = getBTagWeight(sfs,tags);
  ...
  //cout<<"light: "<<nlight<<" "<<nlTagged<<" "<<weightLight<<endl;
  //cout<<"charm: "<<ncquarks<<" "<<ncTagged<<" "<<weightC<<endl;
  //cout<<"bottom: "<<nbquarks<<" "<<nbTagged<<" "<<weightB<<endl;

  //cout<<"bweight ntagged: "<<ntagged<< " 0: "<<m_btagWeights[0]<<" 1: " << m_btagWeights[1]<<" 2: "<<m_btagWeights[2]<<endl;
}
//------------------------------------------
//------------------------------------------
void SignatureHandler::addBjetSignature(Signature* sig, int low, int high)
{
  SignatureWithBjets* bsig = new SignatureWithBjets(*sig,low,high);
  addBjetSignature(bsig);
}
//------------------------------------------
//------------------------------------------
void SignatureHandler::addBjetSignature(const char* name, const char* options, int low, int high)
{
  SignatureWithBjets* sig = new SignatureWithBjets(name,options,low,high);
  addBjetSignature(sig);
}
//------------------------------------------
//------------------------------------------
//------------------------------------------
//------------------------------------------
int SignatureHandler::getBTagWeight(vector<float> scaleFactors,vector<int>tags){
  if(tags.size() != scaleFactors.size())return 0;
  //cout<<nJets<<" "<<(int)scaleFactors.size()<<endl;
  //float weight = 1.0;
  float t0 = 1.0;
  float t1 = 0.0;
  int nTagged = 0;
  for(int i = 0; i < (int)scaleFactors.size(); i++){
    //if(!tags[i])scaleFactors[i] = 1.0;
    if(tags[i]){
      nTagged++;
      t0 *= 1 - scaleFactors[i];
    }
  }
  if(nTagged == 0)return nTagged;
  m_btagWeights[0] = t0;

  for(int i = 0; i < (int)scaleFactors.size(); i++){
    if(!tags[i])continue;
    float term = scaleFactors[i];
    for(int j = 0; j < (int)scaleFactors.size(); j++){
      if(i == j)continue;
      if(!tags[j])continue;
      term *= (1-scaleFactors[j]);
    }
    t1 += term;
  }
  m_btagWeights[1] = t1;
  if(nTagged == 1)return nTagged;
  m_btagWeights[2] = 1 - t0 - t1;
...
//------------------------------------------
//------------------------------------------
vector<float> SignatureHandler::getLightScaleFactors()
{
  vector<float> sfs;
  for(int i = 0; i < (int)m_good_jets.size(); i++){
    if(abs(((SignatureObjectJet*)m_good_jets[i])->getGenParton_pdgID()) != 999 && abs(((SignatureObjectJet*)m_good_jets[i])->getGenParton_pdgID()) != 4 && abs(((SignatureObjectJet*)m_good_jets[i])->getGenParton_pdgID()) != 5){
      SignatureObjectJet* jet = (SignatureObjectJet*)m_good_jets[i];
      //sfs.push_back(m_SFL->Eval(jet->Pt())*m_EFL->Eval(jet->Pt()));
      sfs.push_back(m_SFL->Eval(jet->Pt()));
    }
  }
...
//------------------------------------------
//------------------------------------------
vector<float> SignatureHandler::getCScaleFactors()
{
  vector<float> sfs;
  for(int i = 0; i < (int)m_good_jets.size(); i++){
    if( abs(((SignatureObjectJet*)m_good_jets[i])->getGenParton_pdgID()) == 4){
      SignatureObjectJet* jet = (SignatureObjectJet*)m_good_jets[i];
      float x = 0;
      if(m_bTagForReweight == "CSVL"){
	x = jet->getBDiscPositiveAt(10);
      }else if(m_bTagForReweight == "CSVM"){
	x = jet->getBDiscPositiveAt(10);
      }else if(m_bTagForReweight == "CSVT"){
	x = jet->getBDiscPositiveAt(10);
      }else if(m_bTagForReweight == "JPL"){
	x = jet->getBDiscPositiveAt(5);
      }else if(m_bTagForReweight == "JPM"){
	x = jet->getBDiscPositiveAt(5);
      }else if(m_bTagForReweight == "JPT"){
	x = jet->getBDiscPositiveAt(5);
      }else if(m_bTagForReweight == "TCHPT"){
	x = jet->getBDiscPositiveAt(1);
      }
      sfs.push_back(m_SFB->Eval(x));
      //sfs.push_back(m_SFB->Eval(x)*m_EFC->Eval(x));
    }
  }
...
//------------------------------------------
//------------------------------------------
vector<float> SignatureHandler::getBScaleFactors()
{
  vector<float> sfs;
  for(int i = 0; i < (int)m_good_jets.size(); i++){
    if( abs(((SignatureObjectJet*)m_good_jets[i])->getGenParton_pdgID()) == 5){
      SignatureObjectJet* jet = (SignatureObjectJet*)m_good_jets[i];
      if(m_bTagForReweight == "CSVL"){
	sfs.push_back(m_SFB->Eval(jet->getBDiscPositiveAt(10)));
      }else if(m_bTagForReweight == "CSVM"){
	sfs.push_back(m_SFB->Eval(jet->getBDiscPositiveAt(10)));
      }else if(m_bTagForReweight == "CSVT"){
	sfs.push_back(m_SFB->Eval(jet->getBDiscPositiveAt(10)));
      }else if(m_bTagForReweight == "JPL"){
	sfs.push_back(m_SFB->Eval(jet->getBDiscPositiveAt(5)));
      }else if(m_bTagForReweight == "JPM"){
	sfs.push_back(m_SFB->Eval(jet->getBDiscPositiveAt(5)));
      }else if(m_bTagForReweight == "JPT"){
	sfs.push_back(m_SFB->Eval(jet->getBDiscPositiveAt(5)));
      }else if(m_bTagForReweight == "TCHPT"){
	sfs.push_back(m_SFB->Eval(jet->getBDiscPositiveAt(1)));
      }
    }
  }
...
//------------------------------------------
//------------------------------------------
bool SignatureHandler::isBTagged(SignatureObjectJet* jet)
{
  if(m_bTagForReweight == "CSVL"){
    if(jet->getBDiscPositiveAt(10) > 0.244)return true;
  }else if(m_bTagForReweight == "CSVM"){
    if(jet->getBDiscPositiveAt(10) > 0.679)return true;
  }else if(m_bTagForReweight == "CSVT"){
    if(jet->getBDiscPositiveAt(10) > 0.898)return true;
  }else if(m_bTagForReweight == "JPL"){
    if(jet->getBDiscPositiveAt(5) > 0.275)return true;
  }else if(m_bTagForReweight == "JPM"){
    if(jet->getBDiscPositiveAt(5) > 0.545)return true;
  }else if(m_bTagForReweight == "JPT"){
    if(jet->getBDiscPositiveAt(5) > 0.790)return true;
  }else if(m_bTagForReweight == "TCHPT"){
    if(jet->getBDiscPositiveAt(1) > 3.41)return true;
  }
...
//------------------------------------------
//------------------------------------------
vector<int> SignatureHandler::associateJetsToPartons()
{
  //returning a vector containing parton ID, -1 corresponding to no match

  //first create list of "final" partons: remove protons, t quarks, W, Z
  vector<int> status3list;
  for(int i = 0; i < (int)m_mcparticles.size(); ++i){
    ...
    //remove neutrinos
    if(abs(pdgid) < 20 && abs(pdgid) > 10 && abs(pdgid) % 2 == 0)continue;
    status3list.push_back(i);
  }

  vector<int> associatedParton;
  //associate to a particle within dR < 0.25
  for(int i = 0; i < (int)m_good_jets.size(); ++i){
    float dRmin = 0.251;
    float iMatch = -1;
    for(int j = 0; j < (int)status3list.size(); ++j){
      float dR = m_good_jets[i]->DeltaR(*m_mcparticles[status3list[j]]);
      if(dR < dRmin){
	iMatch = status3list[j];
	dRmin = dR;
      }
    }
    associatedParton.push_back(iMatch);
  }

  return associatedParton;

}
//------------------------------------------------
//------------------------------------------------
TF1* SignatureHandler::GetSFLight(TString meanminmax, TString Atagger, float Etamin, float Etamax)
{
  TF1 *tmpSFl = NULL;
...
// Definition of functions from plot33New.C ----------------------
  ...
  if( Atagger == "CSVL" && sEtamin == "0.0" && sEtamax == "2.4")
    {
      if( meanminmax == "mean" ) tmpSFl = new TF1("SFlight","((1.0344+(0.000962994*x))+(-3.65392e-06*(x*x)))+(3.23525e-09*(x*(x*x)))*(0.979396 + 0.000205898*x + 2.49868e-07*x*x)", 20.,670.);
      if( meanminmax == "min" ) tmpSFl = new TF1("SFlightMin","((0.956023+(0.000825106*x))+(-3.18828e-06*(x*x)))+(2.81787e-09*(x*(x*x)))*(0.979396 + 0.000205898*x + 2.49868e-07*x*x)", 20.,670.);
      if( meanminmax == "max" ) tmpSFl = new TF1("SFlightMax","((1.11272+(0.00110104*x))+(-4.11956e-06*(x*x)))+(3.65263e-09*(x*(x*x)))*(0.979396 + 0.000205898*x + 2.49868e-07*x*x)", 20.,670.);
    }
  ...
  if( Atagger == "CSVM" && sEtamin == "0.0" && sEtamax == "2.4")
    {
      if( meanminmax == "mean" ) tmpSFl = new TF1("SFlight","((1.04318+(0.000848162*x))+(-2.5795e-06*(x*x)))+(1.64156e-09*(x*(x*x)))*(1.10422 + -0.000523856*x + 1.14251e-06*x*x)", 20.,670.);
      if( meanminmax == "min" ) tmpSFl = new TF1("SFlightMin","((0.962627+(0.000448344*x))+(-1.25579e-06*(x*x)))+(4.82283e-10*(x*(x*x)))*(1.10422 + -0.000523856*x + 1.14251e-06*x*x)", 20.,670.);
      if( meanminmax == "max" ) tmpSFl = new TF1("SFlightMax","((1.12368+(0.00124806*x))+(-3.9032e-06*(x*x)))+(2.80083e-09*(x*(x*x)))*(1.10422 + -0.000523856*x + 1.14251e-06*x*x)", 20.,670.);
    }
  ...
  if( Atagger == "CSVT" && sEtamin == "0.0" && sEtamax == "2.4")
    {
      if( meanminmax == "mean" ) tmpSFl = new TF1("SFlight","((0.948463+(0.00288102*x))+(-7.98091e-06*(x*x)))+(5.50157e-09*(x*(x*x)))*(1.19275 + -0.00191042*x + 2.92205e-06*x*x)", 20.,670.);
      if( meanminmax == "min" ) tmpSFl = new TF1("SFlightMin","((0.899715+(0.00102278*x))+(-2.46335e-06*(x*x)))+(9.71143e-10*(x*(x*x)))*(1.19275 + -0.00191042*x + 2.92205e-06*x*x)", 20.,670.);
      if( meanminmax == "max" ) tmpSFl = new TF1("SFlightMax","((0.997077+(0.00473953*x))+(-1.34985e-05*(x*x)))+(1.0032e-08*(x*(x*x)))*(1.19275 + -0.00191042*x + 2.92205e-06*x*x)", 20.,670.);
    }
  ...
  if( Atagger == "JPL" && sEtamin == "0.0" && sEtamax == "2.4")
    {
      if( meanminmax == "mean" ) tmpSFl = new TF1("SFlight","((0.978061+(0.000211142*x))+(-6.67003e-07*(x*x)))+(2.94232e-10*(x*(x*x)))*(1.01744 + 0.000813491*x + -6.01592e-07*x*x)", 20.,670.);
      if( meanminmax == "min" ) tmpSFl = new TF1("SFlightMin","((0.889182+(0.000124259*x))+(-3.83838e-07*(x*x)))+(5.99164e-11*(x*(x*x)))*(1.01744 + 0.000813491*x + -6.01592e-07*x*x)", 20.,670.);
      if( meanminmax == "max" ) tmpSFl = new TF1("SFlightMax","((1.06693+(0.00029804*x))+(-9.50169e-07*(x*x)))+(5.28547e-10*(x*(x*x)))*(1.01744 + 0.000813491*x + -6.01592e-07*x*x)", 20.,670.);
    }
  ...
  if( Atagger == "JPM" && sEtamin == "0.0" && sEtamax == "2.4")
    {
      if( meanminmax == "mean" ) tmpSFl = new TF1("SFlight","((0.871294+(0.00215201*x))+(-6.77675e-06*(x*x)))+(5.79197e-09*(x*(x*x)))*(0.964487 + 0.00134038*x + -1.43995e-06*x*x)", 20.,670.);
      if( meanminmax == "min" ) tmpSFl = new TF1("SFlightMin","((0.7654+(0.00149792*x))+(-4.47192e-06*(x*x)))+(3.67664e-09*(x*(x*x)))*(0.964487 + 0.00134038*x + -1.43995e-06*x*x)", 20.,670.);
      if( meanminmax == "max" ) tmpSFl = new TF1("SFlightMax","((0.977076+(0.00280638*x))+(-9.08158e-06*(x*x)))+(7.9073e-09*(x*(x*x)))*(0.964487 + 0.00134038*x + -1.43995e-06*x*x)", 20.,670.);
    }
  ...
  if( Atagger == "TCHPT" && sEtamin == "0.0" && sEtamax == "2.4")
    {
      if( meanminmax == "mean" ) tmpSFl = new TF1("SFlight","((1.20711+(0.000681067*x))+(-1.57062e-06*(x*x)))+(2.83138e-10*(x*(x*x)))*(1.08376 + -0.000666189*x + 1.01272e-06*x*x)", 20.,670.);
      if( meanminmax == "min" ) tmpSFl = new TF1("SFlightMin","((1.03418+(0.000428273*x))+(-5.43024e-07*(x*x)))+(-6.18061e-10*(x*(x*x)))*(1.08376 + -0.000666189*x + 1.01272e-06*x*x)", 20.,670.);
      if( meanminmax == "max" ) tmpSFl = new TF1("SFlightMax","((1.38002+(0.000933875*x))+(-2.59821e-06*(x*x)))+(1.18434e-09*(x*(x*x)))*(1.08376 + -0.000666189*x + 1.01272e-06*x*x)", 20.,670.);
    }
...
//------------------------------------------------
//------------------------------------------------
TF1* SignatureHandler::GetSFB(TString meanminmax, TString Atagger, float Etamin, float Etamax)
{
  TF1 *tmpSFl = NULL;
  TString sEtamin = Form("%1.1f",Etamin);
  TString sEtamax = Form("%1.1f",Etamax);
...
// Definition of functions from plot33New.C ----------------------
  ...
  if( Atagger == "CSVL" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","-0.113472343605*x +  1.04926963159",0.15,0.95);
  }
  if( Atagger == "CSVM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","-0.113472343605*x +  1.04926963159",0.15,0.95);
  }
  ...
  if( Atagger == "JBPL" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","-0.0567351292981*x+1.08944283264",1.3,5.0);
  }
  if( Atagger == "JBPM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","-0.0567351292981*x+1.08944283264",1.3,5.0);
  }
  ...
  if( Atagger == "JPL" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","-0.193012703295*x+1.05615935852",0.15,1.0);
  }
  if( Atagger == "JPM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","-0.193012703295*x+1.05615935852",0.15,1.0);
  }
  ...
  if( Atagger == "SSVHEM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","0.01190651842*x +  0.93976212582",1.5,4.1);
  }
  if( Atagger == "SSVHPT" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","0.01190651842*x +  0.93976212582",1.5,4.1);
  }
  if( Atagger == "TCHEL" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","0.00152129076412*x +  0.95678084337",1.5,20.0);
  }
  if( Atagger == "TCHEM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","0.00152129076412*x +  0.95678084337",1.5,20.0);
  }
  if( Atagger == "TCHPM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","0.00066342906116*x +  0.93334455507",1.5,20.0);
  }
  if( Atagger == "TCHPT" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("SFB","0.00066342906116*x +  0.93334455507",1.5,20.0);
  }
...
//-----------------------------------------------
TF1* SignatureHandler::GetEffLight(TString meanminmax, TString Atagger, float Etamin, float Etamax)
{
  TF1 *tmpMistag = NULL;
  TString sEtamin = Form("%1.1f",Etamin);
  TString sEtamax = Form("%1.1f",Etamax);
...
//------------------------------------------------
TF1* SignatureHandler::GetEffC(TString meanminmax, TString Atagger, float Etamin, float Etamax)
{
  TF1 *tmpSFl = NULL;
  TString sEtamin = Form("%1.1f",Etamin);
  TString sEtamax = Form("%1.1f",Etamax);
...
// Definition of functions from plot33New.C ----------------------
  ...
  if( Atagger == "CSVL" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","-1.5734604211*x*x*x*x +  1.52798999269*x*x*x +  0.866697059943*x*x +  -1.66657942274*x +  0.780639301724",0.15,0.95);
  }
  if( Atagger == "CSVM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","-1.5734604211*x*x*x*x +  1.52798999269*x*x*x +  0.866697059943*x*x +  -1.66657942274*x +  0.780639301724",0.15,0.95);
  }
  ...
  if( Atagger == "JBPL" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","-0.00306098884977*x*x*x*x+0.0226880063933*x*x*x+0.00680649433831*x*x-0.416353599879*x+0.919767036704",1.3,5.0);
  }
  if( Atagger == "JBPM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","-0.00306098884977*x*x*x*x+0.0226880063933*x*x*x+0.00680649433831*x*x-0.416353599879*x+0.919767036704",1.3,5.0);
  }
  ...
  if( Atagger == "JPL" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","-1.31691538262*x*x*x*x+2.18754138484*x*x*x-0.0360042485486*x*x-1.71795332635*x+0.86169401499",0.15,1.0);
  }
  if( Atagger == "JPM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","-1.31691538262*x*x*x*x+2.18754138484*x*x*x-0.0360042485486*x*x-1.71795332635*x+0.86169401499",0.15,1.0);
  }
  ...
  if( Atagger == "SSVHEM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","0.00143789244176*x*x*x*x +  -0.00186154098096*x*x*x +  -0.0230147611829*x*x +  -0.0497041720896*x +  0.324134245921",1.5,4.1);
  }
  if( Atagger == "SSVHPT" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","0.000538610179605*x*x*x*x +  -0.000832382239052*x*x*x +  -0.00911037101565*x*x +  -0.0179746431461*x +  0.135656230325",1.5,4.1);
  }
  if( Atagger == "TCHEL" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","0.343760640168*exp(-0.00315525164823*x*x*x + 0.0805427315196*x*x + -0.867625139194*x + 1.44815935164 )",1.5,20.0);
  }
  if( Atagger == "TCHEM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","0.343760640168*exp(-0.00315525164823*x*x*x + 0.0805427315196*x*x + -0.867625139194*x + 1.44815935164 )",1.5,20.0);
  }
  if( Atagger == "TCHPM" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","0.451288118581*exp(-0.0213290505241*x*x*x + 0.356020789904*x*x + -2.20158883207*x + 1.84838018633 )",1.5,20.0);
  }
  if( Atagger == "TCHPT" && sEtamin == "0.0" && sEtamax == "2.4"){
    tmpSFl = new TF1("effc","0.451288118581*exp(-0.0213290505241*x*x*x + 0.356020789904*x*x + -2.20158883207*x + 1.84838018633 )",1.5,20.0);
  }
